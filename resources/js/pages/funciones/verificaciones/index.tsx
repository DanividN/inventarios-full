...
export default function Index() {
    const { areasResguardos } = usePage().props;
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                accessorKey: 'id',
                label: 'Acciones',
                cell: (info: any) => <ActionMenu to={`/funciones/verificaciones/resguardatarios/${info.row.original.id}`} text={'Ver'} />,
                disableFilter: true,
            },
        ],
        [],
    );
    const data = areasResguardos.map((resguardoAsignado) => ({
